<template>
  <div class="min-h-screen bg-gray-50 dark:bg-gray-950">
    ...
    <!-- Bottom navigation -->
    <div class="fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-900 flex justify-around py-2 z-50 border-t border-gray-200 dark:border-gray-800">
      <!-- Главная -->
      <UButton color="neutral" variant="ghost" class="flex flex-col items-center text-gray-400 gap-0 h-auto py-1 px-0 min-w-0" @click="goToHome">
        <UIcon name="i-heroicons-home" class="w-6 h-6" />
        <span class="text-[10px]">Главная</span>
      </UButton>

      <!-- Задачи -->
      <UButton color="neutral" variant="ghost" class="flex flex-col items-center text-gray-400 gap-0 h-auto py-1 px-0 min-w-0 relative" @click="goToTasks">
      ...
      <!-- Каталог (Активный) -->
      <UButton color="neutral" variant="ghost" class="flex flex-col items-center text-black dark:text-white gap-0 h-auto py-1 px-0 min-w-0" @click="goToCatalog">
        <UIcon name="i-heroicons-squares-2x2" class="w-6 h-6" />
        <span class="text-[10px]">Каталог</span>
      </UButton>

      <!-- Корзина (с бейджем) -->
      <UButton color="neutral" variant="ghost" class="flex flex-col items-center text-gray-400 gap-0 h-auto py-1 px-0 min-w-0 relative" @click="goToCart">
        <UIcon name="i-heroicons-shopping-cart" class="w-6 h-6" />
        <span v-if="cart.totalCount > 0" class="absolute -top-1 -right-1 flex items-center justify-center w-4 h-4 bg-[#B3261E] text-white text-[10px] rounded-full">
          {{ cart.totalCount }}
        </span>
...
const cart = useCartStore() // Подключаем стор

const searchQuery = ref('')

const products = ref([
  { id: '100001', name: 'Coca-Cola 0.5L', price: '89', quantity: '24', image: '/phone.jpg' },
  { id: '100002', name: 'Pepsi 0.5L', price: '79', quantity: '18', image: '/phone.jpg' },
  { id: '100003', name: 'Lay’s Classic', price: '129', quantity: '32', image: '/phone.jpg' },
  { id: '100004', name: 'Snickers', price: '99', quantity: '41', image: '/phone.jpg' },
  { id: '100005', name: 'Kinder Chocolate', price: '119', quantity: '27', image: '/phone.jpg' },
  { id: '100006', name: 'Red Bull 0.25L', price: '149', quantity: '15', image: '/phone.jpg' }
])
...
const addToCart = (product: any) => {
  cart.add(product) // Добавляем в стор
}
...
<style>
input { outline: none !important; outline-style: none !important; box-shadow: none !important; }
</style>