<template>
  <div class="min-h-screen bg-gray-50 dark:bg-[#0a0a0a] pb-24">
    ...
    <div class="max-w-7xl w-full bg-gray-50 dark:bg-[#0a0a0a] mx-auto min-h-screen p-5 pb-24">
      <!-- Мои / Филиала -->
      <div class="flex items-center justify-between gap-2 px-4 sm:px-6 lg:px-8 mb-[14px]">
        ...
        <UButton
          color="primary"
          class="h-[30px] md:h-[34px] px-2.5 md:px-3 text-[11px] md:text-xs font-semibold rounded-lg whitespace-nowrap flex items-center justify-center bg-[#00C16A] hover:bg-[#00a85a] text-white border-none dark:bg-[#00C16A] dark:hover:bg-[#00a85a]"
          @click="createTask"
        >
          + Создать
        </UButton>
      ...
      <!-- Поиск -->
      <div class="px-4 sm:px-6 lg:px-8 mb-[14px] w-full">
        <UInput
          v-model="searchQuery"
          placeholder="Search..."
          class="w-full bg-[#f2f2f7] dark:bg-[#1a1a1a] rounded-xl border-transparent text-black dark:text-white placeholder-[#8e8e93] dark:placeholder-gray-500 text-sm focus:border-[#8a5af0] dark:focus:border-[#8a5af0] focus:ring-2 focus:ring-[#8a5af0]/20 dark:focus:ring-[#8a5af0]/20 transition-all duration-200"
        >
          <template #leading>
      ...
      <!-- Фильтры -->
      <div class="flex gap-2.5 px-4 sm:px-6 lg:px-8 mb-4 relative" style="z-index: 60;">
        <!-- Статус -->
        <div class="flex-1 relative" :class="{ 'opacity-40 pointer-events-none': priorityDropdownOpen }" style="z-index: 61;">
          <UButton
            color="gray"
            variant="ghost"
            class="w-full justify-between bg-[#f2f2f7] dark:bg-[#1a1a1a] rounded-lg px-3 py-1.5 text-sm font-medium text-black dark:text-white hover:bg-[#e8e8ed] dark:hover:bg-[#2a2a2a] transition-colors duration-200"
          ...
          <div
            ...
            class="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-[#1a1a1a] rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 z-50 overflow-hidden animate-fadeIn"
          >
            <div
              v-for="item in statusItems"
              :key="item.value"
              class="flex items-center gap-2 px-3 py-2 hover:bg-gray-100 dark:hover:bg-[#2a2a2a] cursor-pointer transition-colors duration-150"
      ...
      <!-- Сегодня -->
      <div class="text-base font-bold text-black dark:text-white px-4 sm:px-6 lg:px-8 mb-3 transition-colors duration-200">Сегодня</div>

      <div 
        v-for="(task, idx) in todayTasks" 
        :key="'today-' + idx"
        class="cursor-pointer px-4 sm:px-6 lg:px-8"
      ...
      <!-- Завтра -->
      <div class="text-base font-bold text-black dark:text-white px-4 sm:px-6 lg:px-8 mb-3 mt-6 transition-colors duration-200">Завтра</div>
...
const todayTasks = reactive([
  { id: 1, type: 'Выдача заказа', number: '1947', name: 'Выдать заказ №12134', status: 'Выполнено', execLabel: 'Средний', time: 'Сегодня, 15:00', sub: 'создана сегодня, 10:12', done: true, statusColor: 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300' },
  { id: 2, type: 'Резервирование', number: '19857', name: 'Отложить iPhone 16 Pro Max', status: 'В работе', execLabel: 'Срочный', time: 'Сегодня, 17:00', sub: 'создана сегодня, 14:12', done: false, statusColor: 'bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300' }
])

const tomorrowTasks = reactive([
  { id: 3, type: 'Инвентаризация', number: '77777', name: 'Пересчитать товар арт. 12784568', status: 'В работе', execLabel: 'Низкий', time: 'Завтра, 09:00', sub: 'создана сегодня, 08:00', done: false, statusColor: 'bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300' },
  { id: 4, type: 'Инвентаризация', number: '77777', name: 'Пересчитать товар арт. 12784568', status: 'Новая', execLabel: 'Низкий', time: 'Завтра, 09:00', sub: 'создана сегодня, 08:00', done: false, statusColor: 'bg-gray-100 dark:bg-gray-800/50 text-gray-800 dark:text-gray-300' }
])
